...
export default function AdminChatPanel() {
  ...
  const activeUser = currentUser
    ? userMessages.find((u) => u.id === currentUser.id)
    : null;
          ...
          if (userExists) {
            return prev.map((u) =>
              u.id === newMessage.sender
                ? {
                    ...u,
                    unread: currentUserRef.current?.id !== newMessage.sender,
  ...
  const fetchNewUserMessages = async (id) => {
    try {
      const data = await apiFetch(`/adminRoute/messages/${id}`);
      if (data.result === 'success') {
        setUserMessages((prev) => [...prev, data.data[0]]);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleUserClick = async (user) => {
    ...
    if (user.unread) {
      try {
        await apiFetch(`/adminRoute/readmessages/${user.id}`, {
          method: 'PATCH',
        });
        setUserMessages((prev) =>
  ...
  const getGroupedUsers = () => {
    ...
    userMessages.forEach((u) => {
      const t =
        u.chatTopic && predefinedTopics.includes(u.chatTopic)
          ? u.chatTopic
          : 'Egyéb';
      groups[t].push(u);
    });
        ...
        <div className={`adminChatList ${isSideBarOpen ? 'open' : ''}`}>
          {predefinedTopics.map((topic) => (
            <div key={topic} className="topicAccordion">
              <div
                className="topicAccordionHeader"
                onClick={() => toggleTopic(topic)}
              >
                <span>
                  {topic} ({groupedUsers[topic].length})
                </span>
                <span>{openTopics[topic] ? '▲' : '▼'}</span>
              </div>

              <div
                className={`topicAccordionBody ${openTopics[topic] ? 'open' : ''}`}
              >
                {groupedUsers[topic].length > 0 ? (
                  ...
                  <p className="adminEmptyTopic">Nincs beszélgetés</p>
                )}
              </div>
            </div>
          ))}
          ...
          <div className="adminChatText">
            <div className="adminChatMessageField">
              {activeUser.messages.map((message, idx) => {
                const d = new Date(
                  message.date || message.sentAt || Date.now()
                );
                ...
                return (
                  <div
                    key={idx}
                    className={`adminMessageWrapper ${isSent ? 'sent' : 'received'}`}
                  >
                    <div
                      className={
                        isSent ? 'sentMessageAdmin' : 'receivedMessageAdmin'
                      }
                    >
                      {message.text}
                    </div>
