...
export default function ReviewModal({ product, onClose }) {
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/reviews/${product.prodId}`)
            .then(res => res.json())
            .then(data => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if(response.ok && result.result === 'success') {
                setComment('');
                const newReviewRes = await fetch(`http://localhost:3000/reviews/${product.prodId}`);
        ...
        <div className="reviewModalOverlay" onClick={onClose}>
            <div className="reviewModalContent" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="addReviewSection">
                    ...
                    />
                    <button onClick={handleSubmit} className="submitReviewBtn">Küldés</button>
                ...
                <div className="reviewsList">
                    ...
                    {reviews.map(review => (
                        <div key={review.reviewId} className="reviewItem">
                            ...
                            <small className="reviewDate">{new Date(review.reviewDate).toLocaleDateString()}</small>
                        </div>
                    ))}
                </div>
            </div>
